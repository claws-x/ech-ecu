 ...
 * @brief 滑动平均滤波
 */
static uint16_t ApplyFilter(EchAdcController_t* controller, EchAdcChannelId_t channelId, uint16_t newValue);
...
int32_t EchAdc_Init(EchAdcController_t* controller)
{
    ...
    for (int i = 0; i < ECH_ADC_CH_MAX; i++) {
        ...
        controller->channels[i].enabled = (i < ECH_ADC_CHANNEL_COUNT) ? true : false;
...
int32_t EchAdc_Sample(EchAdcController_t* controller, uint32_t timestamp_ms)
{
    ...
    for (int i = 0; i < ECH_ADC_CH_MAX; i++) {
        ...
        controller->rawData[i].validFlag = (status == ECH_ADC_STATUS_OK) ? ECH_ADC_DATA_VALID : ECH_ADC_DATA_INVALID;
        ...
        if (status == ECH_ADC_STATUS_OK) {
            engineeredValue = ApplyCalibration(
                engineeredValue,
                controller->channels[i].calibrationOffset,
...
{
    if (major != NULL) *major = ECH_ADC_VERSION_MAJOR;
    if (minor != NULL) *minor = ECH_ADC_VERSION_MINOR;
    if (patch != NULL) *patch = ECH_ADC_VERSION_PATCH;
}
...
static float RawToTemperature(uint16_t rawValue)
{
    ...
    float rNtc = ECH_NTC_SERIES_RESISTANCE * vOut / (ECH_ADC_REF_VOLTAGE_MV - vOut);
...
/* 模拟设置原始值用于测试 */
static void SetRawValue(EchAdcController_t* controller, EchAdcChannelId_t channelId, uint16_t value)
{
...
void test_adc_calibration(void)
{
    ...
    printf("  Raw temp=%.2f°C, Calibrated temp=%.2f°C (offset=+2°C)\n", temp_raw, temp_calibrated);
    
    /* 校准后的温度应该比原始值高约 2°C */
    assert((temp_calibrated - temp_raw) > 1.5f && (temp_calibrated - temp_raw) < 2.5f);
