 ...
 */
static void BuildLinFrame(EchLinFrame_t* frame, uint8_t id, const uint8_t* data, uint8_t length);
 ...
 * @brief 处理心跳超时
 */
static void ProcessHeartbeatTimeout(EchLinController_t* controller, uint32_t timestamp_ms);
 ...
 * @brief 初始化 LIN 控制器
 */
int32_t EchLin_Init(EchLinController_t* controller, const EchLinConfig_t* config)
{
    /* 参数检查 */
    if (controller == NULL) {
        return -1;
 ...
 * @brief 发送 LIN 帧
 */
int32_t EchLin_SendFrame(EchLinController_t* controller, const EchLinFrame_t* frame)
{
 ...
 * @brief 接收 LIN 帧
 */
int32_t EchLin_ReceiveFrame(EchLinController_t* controller, EchLinFrame_t* frame, uint32_t timestamp_ms)
{
    if (controller == NULL || !controller->initialized || frame == NULL) {
        return -1;
    }
 ...
 * @brief 发送心跳帧
 */
int32_t EchLin_SendHeartbeat(EchLinController_t* controller, uint32_t timestamp_ms)
{
    if (controller == NULL || !controller->initialized) {
        return -1;
    }
 ...
 * @brief 发送状态帧
 */
int32_t EchLin_SendStatus(EchLinController_t* controller, const uint8_t* statusData, uint32_t timestamp_ms)
{
 ...
 * @brief 接收命令帧
 */
int32_t EchLin_ReceiveCommand(EchLinController_t* controller, uint8_t* commandData, uint32_t timestamp_ms)
{
 ...
 * @brief 获取诊断信息
 */
void EchLin_GetDiagnostic(const EchLinController_t* controller, EchLinDiagnostic_t* diagnostic)
{
 ...
 * - 取反码
 */
uint8_t EchLin_CalculateChecksum(const uint8_t* data, uint8_t length, uint8_t id)
{
    uint16_t sum = 0;
...
{
    if (major != NULL) *major = ECH_LIN_VERSION_MAJOR;
    if (minor != NULL) *minor = ECH_LIN_VERSION_MINOR;
    if (patch != NULL) *patch = ECH_LIN_VERSION_PATCH;
}
...
static bool VerifyChecksum(const EchLinFrame_t* frame)
{
    ...
    uint8_t calculatedChecksum = EchLin_CalculateChecksum(
        frame->data, 
        frame->length, 
        frame->id
    );
...
static void ProcessHeartbeatTimeout(EchLinController_t* controller, uint32_t timestamp_ms)
{
    ...
    if (!controller->config.isMaster) {
        if (controller->lastHeartbeat_ms > 0) {
            if (timestamp_ms - controller->lastHeartbeat_ms > ECH_LIN_TIMEOUT_HEARTBEAT) {
...
void test_lin_diagnostic(void)
{
    ...
    printf("  Diagnostic: state=%d, frames=%u, errors=%u\n", 
           diag.state, diag.frameCount, diag.errorCount);
