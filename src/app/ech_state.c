 ...
 * @brief 验证状态转换是否合法
 */
static bool IsValidTransition(EchSystemState_t fromState, EchSystemState_t toState);

/**
 * @brief 执行状态进入动作
 */
static void OnStateEnter(EchStateController_t* controller, EchSystemState_t newState, uint32_t timestamp_ms);

/**
 * @brief 执行状态退出动作
 */
static void OnStateExit(EchStateController_t* controller, EchSystemState_t oldState);

/**
 * @brief 状态机逻辑处理
 */
static void ProcessStateLogic(EchStateController_t* controller, uint32_t timestamp_ms);
 ...
 * @brief 记录故障到历史
 */
static void RecordFault(EchStateController_t* controller, EchFaultType_t faultType, EchSystemState_t state, float value, uint32_t timestamp_ms);
...
{
    if (controller == NULL || !controller->initialized || faultType == ECH_FAULT_NONE) {
        return -1;
    }
    ...
    if (firstFault) {
        ...
        /* 第一次故障，跳过去抖，立即报告 */
    } else if (currentTime - controller->faultDebounceTimer_ms < ECH_STATE_FAULT_DEBOUNCE_MS) {
        return 0;  /* 去抖中，忽略 */
    }
    ...
    /* 记录故障 */
    RecordFault(controller, faultType, controller->currentState, currentValue, currentTime);
...
int32_t EchState_ClearFault(EchStateController_t* controller, EchFaultType_t faultType)
{
    ...
    /* 查找并清除故障 */
    for (int i = 0; i < controller->faultCount && i < ECH_STATE_MAX_FAULT_HISTORY; i++) {
        if (controller->faultHistory[i].faultType == faultType && !controller->faultHistory[i].isCleared) {
...
const EchFaultRecord_t* EchState_GetFaultHistory(const EchStateController_t* controller, uint8_t* count)
{
    ...
    *count = (controller->faultCount < ECH_STATE_MAX_FAULT_HISTORY) ? 
             controller->faultCount : ECH_STATE_MAX_FAULT_HISTORY;
...
{
    switch (state) {
        case ECH_STATE_INIT:       return "INIT";
        case ECH_STATE_STANDBY:    return "STANDBY";
        case ECH_STATE_RUNNING:    return "RUNNING";
        case ECH_STATE_FAULT:      return "FAULT";
        case ECH_STATE_SHUTDOWN:   return "SHUTDOWN";
...
{
    if (major != NULL) *major = ECH_STATE_VERSION_MAJOR;
    if (minor != NULL) *minor = ECH_STATE_VERSION_MINOR;
    if (patch != NULL) *patch = ECH_STATE_VERSION_PATCH;
}
...
static bool IsValidTransition(EchSystemState_t fromState, EchSystemState_t toState)
{
    ...
    switch (fromState) {
        ...
        case ECH_STATE_STANDBY:
            return (toState == ECH_STATE_RUNNING || toState == ECH_STATE_INIT || toState == ECH_STATE_SHUTDOWN);
        
        case ECH_STATE_RUNNING:
            return (toState == ECH_STATE_STANDBY || toState == ECH_STATE_FAULT || toState == ECH_STATE_SHUTDOWN);
...
{
    /* 使用传入的时间戳作为状态进入时间 */
    controller->stateEntryTime_ms = (timestamp_ms > 0) ? timestamp_ms : controller->stateEntryTime_ms;
    controller->stateDuration_ms = 0;
 ...
 * @brief 记录故障到历史
 */
static void RecordFault(EchStateController_t* controller, EchFaultType_t faultType, 
                        EchSystemState_t state, float value, uint32_t timestamp_ms)
{
...
void test_state_transition(void)
{
    ...
    printf("  After init: state=%d, entryTime=%u\n", state.currentState, state.stateEntryTime_ms);
