 ...
 * 
 * @details
 * 本文件是 ECH 水加热器 ECU 的主程序入口，负责系统初始化、任务调度和主循环管理。
 * 
...
int main(void)
{
    ...
    /* Step 5: 切换到待机状态 */
    EchState_RequestTransition(&g_system.state, ECH_STATE_STANDBY, GetTimestamp_ms());
...
static void Task_10ms(void)
{
    ...
    /* 获取温度值 */
    float inletTemp = EchAdc_GetTemperature(&g_system.adc, ECH_ADC_CH_TEMP_INLET);
    float outletTemp = EchAdc_GetTemperature(&g_system.adc, ECH_ADC_CH_TEMP_OUTLET);
...
static void Task_100ms(void)
{
    ...
    float targetTemp = 50.0f;  /* 默认目标温度 */
    
    /* 获取实际温度 */
    float currentTemp = EchAdc_GetTemperature(&g_system.adc, ECH_ADC_CH_TEMP_OUTLET);
    
    /* PID 控制计算 */
    if (EchState_IsRunning(&g_system.state)) {
        float pwmDuty = EchPid_Calculate(&g_system.pid, currentTemp, g_system.timestamp_ms);
        
        /* 设置 PWM 输出 */
        /* EchPwm_SetDuty(pwmDuty); */
    }
}
...
static void Task_1000ms(void)
{
    ...
    /* 检查传感器故障 */
    EchAdcStatus_t adcStatus = EchAdc_GetChannelStatus(&g_system.adc, ECH_ADC_CH_TEMP_OUTLET);
...
static void System_Shutdown(void)
{
    ...
    /* 进入关机状态 */
    EchState_RequestTransition(&g_system.state, ECH_STATE_SHUTDOWN, g_system.timestamp_ms);
...
{
    if (major != NULL) *major = ECH_MAIN_VERSION_MAJOR;
    if (minor != NULL) *minor = ECH_MAIN_VERSION_MINOR;
    if (patch != NULL) *patch = ECH_MAIN_VERSION_PATCH;
}
