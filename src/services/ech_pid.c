 ...
 * 
 * @details
 * 本文件实现 PID (比例 - 积分 - 微分) 控制算法，用于 ECH 水加热器的温度闭环控制。
 * 
 ...
 * @brief 初始化 PID 控制器
 */
int32_t EchPid_Init(EchPidController_t* controller, const EchPidConfig_t* config)
{
    /* 参数检查 */
 ...
 * @brief 执行 PID 控制计算
 */
float EchPid_Calculate(EchPidController_t* controller, float measured_value, uint32_t timestamp_ms)
{
    /* 参数检查 */
    if (controller == NULL) {
        return 0.0f;
    ...
    controller->integral = Clamp(controller->integral, ECH_PID_INTEGRAL_MIN, ECH_PID_INTEGRAL_MAX);
    
    /* 检测积分饱和 */
    controller->integral_saturated = (controller->integral >= ECH_PID_INTEGRAL_MAX) ||
    ...
    controller->prev_derivative = ECH_PID_DERIVATIVE_FILTER * derivative_raw +
    ...
    controller->output = Clamp(controller->output, ECH_PID_OUTPUT_MIN, ECH_PID_OUTPUT_MAX);
 ...
 * @brief 获取 PID 诊断信息
 */
void EchPid_GetDiagnostic(EchPidController_t* controller, EchPidDiagnostic_t* diagnostic)
{
    ...
    diagnostic->is_saturated = controller->integral_saturated || controller->output_saturated;
...
{
    if (major != NULL) *major = ECH_PID_VERSION_MAJOR;
    if (minor != NULL) *minor = ECH_PID_VERSION_MINOR;
    if (patch != NULL) *patch = ECH_PID_VERSION_PATCH;
}
