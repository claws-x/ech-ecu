...
int32_t EchWdg_Start(EchWdgController_t* controller, uint32_t timestamp_ms)
{
    ...
    controller->nextFeedDeadline_ms = timestamp_ms + controller->config.timeout_ms;
...
int32_t EchWdg_Update(EchWdgController_t* controller, uint32_t timestamp_ms)
{
    ...
    if (timeToTimeout == 0 || timeToTimeout > controller->config.timeout_ms) {
        ...
        for (uint8_t i = 0; i < controller->taskCount; i++) {
            ...
            if (task->enabled) {
                uint32_t taskTimeToTimeout = (task->lastFeed_ms + task->timeout_ms) - timestamp_ms;
...
void EchWdg_GetDiagnostic(const EchWdgController_t* controller, uint32_t timestamp_ms, EchWdgDiagnostic_t* diagnostic)
{
    ...
    diagnostic->timeToTimeout_ms = EchWdg_GetTimeToTimeout(controller, timestamp_ms);
...
{
    if (major != NULL) *major = ECH_WDG_VERSION_MAJOR;
    if (minor != NULL) *minor = ECH_WDG_VERSION_MINOR;
    if (patch != NULL) *patch = ECH_WDG_VERSION_PATCH;
}
...
void test_wdg_diagnostic(void)
{
    ...
    printf("  Diagnostic: state=%d, feeds=%u, healthy=%d\n", 
           diag.state, diag.totalFeedCount, diag.isHealthy);
