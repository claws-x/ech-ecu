 ...
 * @brief 查找 DTC 记录
 */
static EchDtcRecord_t *FindDtcRecord(EchDiagController_t *controller,
                                     EchDtcCode_t dtcCode);

/**
 * @brief 处理诊断会话控制请求
 */
static int32_t ProcessDiagnosticSessionControl(EchDiagController_t *controller,
                                               uint8_t subFunction,
 ...
 * @brief 处理读取 DTC 请求
 */
static int32_t ProcessReadDtc(EchDiagController_t *controller,
                              uint8_t subFunction, EchDiagResponse_t *response);

/**
 * @brief 处理清除 DTC 请求
 */
static int32_t ProcessClearDtc(EchDiagController_t *controller,
 ...
 * @brief 处理读取 DID 请求
 */
static int32_t ProcessReadDid(EchDiagController_t *controller, uint16_t didId,
                              EchDiagResponse_t *response);
 ...
 * @brief 初始化诊断控制器
 */
int32_t EchDiag_Init(EchDiagController_t *controller,
                     const EchDiagConfig_t *config) {
  /* 参数检查 */
  if (controller == NULL) {
    return -1;
  }
 ...
 * @brief 处理 UDS 请求
 */
int32_t EchDiag_ProcessRequest(EchDiagController_t *controller,
                               const uint8_t *requestData,
                               uint16_t requestLength,
                               EchDiagResponse_t *response,
                               uint32_t timestamp_ms) {
  if (controller == NULL || !controller->initialized || requestData == NULL ||
      response == NULL) {
    return -1;
  }
  ...
  case ECH_UDS_SID_DIAG_CTRL: /* 0x10: 诊断会话控制 */
    ...
    return ProcessDiagnosticSessionControl(controller, requestData[1], response,
                                           timestamp_ms);
 ...
 * @brief 设置 DTC 故障码
 */
int32_t EchDiag_SetDtc(EchDiagController_t *controller, EchDtcCode_t dtcCode,
                       const uint8_t *freezeFrame) {
  if (controller == NULL || !controller->initialized ||
      dtcCode == ECH_DTC_NONE) {
    return -1;
  }
 ...
 * @brief 清除 DTC 故障码
 */
int32_t EchDiag_ClearDtc(EchDiagController_t *controller,
                         EchDtcCode_t dtcCode) {
  if (controller == NULL || !controller->initialized) {
    return -1;
  }
  ...
  for (uint8_t i = 0; i < controller->dtcCount - 1; i++) {
    if (controller->dtcRecords[i].dtcCode == dtcCode) {
      controller->dtcRecords[i] =
          controller->dtcRecords[controller->dtcCount - 1];
      break;
    }
  }
  controller->dtcCount--;
 ...
 * @brief 获取 DTC 记录
 */
const EchDtcRecord_t *
EchDiag_GetDtcRecord(const EchDiagController_t *controller,
                     EchDtcCode_t dtcCode) {
  if (controller == NULL || !controller->initialized) {
    return NULL;
  }
 ...
 * @brief 获取所有 DTC 列表
 */
uint8_t EchDiag_GetAllDtcList(const EchDiagController_t *controller,
                              EchDtcCode_t *dtcList, uint8_t maxCount) {
  if (controller == NULL || !controller->initialized || dtcList == NULL) {
    return 0;
  }

  uint8_t count =
      (controller->dtcCount < maxCount) ? controller->dtcCount : maxCount;
 ...
 * @brief 切换诊断会话
 */
int32_t EchDiag_ChangeSession(EchDiagController_t *controller,
                              EchDiagSessionType_t sessionType,
                              uint32_t timestamp_ms) {
  if (controller == NULL || !controller->initialized) {
    return -1;
  }
...
EchDiag_GetCurrentSession(const EchDiagController_t *controller) {
  if (controller == NULL || !controller->initialized) {
 ...
 * @brief 获取诊断统计信息
 */
void EchDiag_GetDiagnostic(const EchDiagController_t *controller,
                           uint32_t *requestCount, uint32_t *errorCount,
                           uint8_t *dtcCount) {
  if (controller == NULL || !controller->initialized) {
    return;
  }
...
void EchDiag_GetVersion(uint8_t *major, uint8_t *minor, uint8_t *patch) {
  if (major != NULL)
    *major = ECH_DIAG_VERSION_MAJOR;
  if (minor != NULL)
    *minor = ECH_DIAG_VERSION_MINOR;
  if (patch != NULL)
    *patch = ECH_DIAG_VERSION_PATCH;
}
...
static int32_t ProcessReadDtc(EchDiagController_t *controller,
  ...
  if (subFunction == 0x02) { /* 读取所有 DTC */
    response->dataLength =
        1 + (controller->dtcCount * 4); /* DTC 数量 + 每个 DTC 3 字节 */
...
static int32_t ProcessReadDid(EchDiagController_t *controller, uint16_t didId,
  ...
  int32_t result =
      EchDiag_ReadDid(controller, didId, &response->data[2], &dataLength);
...
void test_diag_init(void) {
  ...
  EchDiagConfig_t config = {.sessionTimeout_ms = 5000,
                            .p2ServerTime_ms = 50,
                            .securityEnabled = false};

  int32_t result = EchDiag_Init(&diag, &config);
  if (result != 0) {
    printf("FAIL: init result\n");
    return;
  }
  if (!diag.initialized) {
    printf("FAIL: initialized\n");
    return;
  }
